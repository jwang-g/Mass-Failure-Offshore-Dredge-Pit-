...
L = zeros(size(zew));
for i = 1:size(zew)
    [l,e,c] = wavelength(wave_period, zew(i),30*pi/180,0.25,150*pi/180);
    L(i) = l;
end
...
d = zeros(size(zew));
for i = 1:size(zew)
    d(i) = w_height(L(i), zew(i), deepwater_wave_height);
end
...
% input.
beta = abs(gradient(zew) ./ gradient(yew));
d = beta .* x; 
...
%           .* (B - beta / (3 * k)) ./ A;
      delta_p_EW = (k * gamma * L * 4 * pi^2) .* (x ./ L).^3 ...
          .* (B - beta / (3 * k)) ./ A;


%% Quick plot

%plotyy(yew, -zew, yew, delta_p_EW);
%show()
