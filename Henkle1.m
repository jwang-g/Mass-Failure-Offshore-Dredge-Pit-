...
k=0.04; 
gamma=0.7;
wave_period = 6; %Seconds - Typical of shelf...
deepwater_wave_height = 10; % Meters... Just guessing!
...
data = importdata('XYZ_for_Henkle.txt');
data = data.data;
y = data(:,2);
z = data(:,3);
...
L = zeros(size(z));
for i = 1:size(z)
    [l,e,c] = wavelength(wave_period, z(i), 0, 0, 0);
    L(i) = l;
end
